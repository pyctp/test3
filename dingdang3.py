#!/usr/bin/env python
# -*- coding: utf-8 -*-
# dingdang3, v 0.1 2018-08-25, first draft
...
from myfunction import LLV, HHV, HLV


from Constant import LOGS_DIR

from myfunction import HHV, LLV, MID, HLV

import queue

from getaccount import getAccountinfo

# BROKER_ID, INVESTOR_ID, PASSWORD, ADDR_MD, ADDR_TD = getAccountinfo('simnow_ctp.json')
BROKER_ID, INVESTOR_ID, PASSWORD, ADDR_MD, ADDR_TD = getAccountinfo('dfqhthm_ctp.json')

inst = ['rb1901']

strategy_paras = {'inst':'rb1901', 'interval':900, 'bkvol':5, 'skvol':5, 'grid':160, 'single_vol':1}

strategy_inst = strategy_paras['inst']
strategy_period = strategy_paras['interval']
...
def dingdang_three():
    ...
    while True:
        if not q_depth_market_data.empty():
            ...
            last_k_line = get_last_k_line(instrument_id=inst[0], interval=strategy_period)
            ...
            if last_k_line is not None and tick.InstrumentID == strategy_inst:
                ...
                print('当前时间: ', tick.UpdateTime, 'HIGH:', last_k_line['high'], '    ', 'LOW:', last_k_line['low'], '当前价: ', tick.LastPrice, '叮当值:', Mid[-1], dingdang)
                ...
                if ups[-1] and not up_trader_flag:
                    ...
                    print('lastHigh:', last_k_line['high'], 'lastLow: ',last_k_line['low'], 'lastPrice: ', tick.LastPrice)
