...
BROKER_ID, INVESTOR_ID, PASSWORD, ADDR_MD, ADDR_TD = getAccountinfo(r'conf\tgjy_ctp_chenmei.json')
...
strategy_paras = {'inst': 'rb1910', 'interval': 300, 'bkvol': 2, 'skvol': 2, 'grid': 8, 'single_vol': 1}
...
def MovingBreak():
    ...
    # 登录行情服务器
    md = MyMdApi(instruments=['rb1910'], broker_id=BROKER_ID, investor_id=INVESTOR_ID, password=PASSWORD)
    md.Create(INVESTOR_ID + "data")
    md.RegisterFront(ADDR_MD)
    md.Init()
    ...
    while True:
        ...
        if not q_depth_market_data.empty():
            ...
            bk_limit = 2 if C[-1] > Mid[-1] else 1
            # bk_limit = 0
            sk_limit = 2 if C[-1] < Mid[-1] else 1
            # sk_limit = 0
