...
# ADDR_MD = 'tcp://101.230.3.50:19000'
# ADDR_TD = 'tcp://101.230.3.50:18000'
    ...
    while True:
        if not q_depth_market_data.empty():
            ...
            if last_k_line is not None and tick.InstrumentID == strategy_inst:
                ...
                if (not up_trader_flag and not last_k_line['low'] == last_k_line['high']) and tick.LastPrice > last_k_line['high'] and buy_order <= buy_limit:
                    # 下多单
                    up_trader_flag = True
                    ...
                    td.PrepareOrder(tick.InstrumentID, b, k, single_volume, tick.LastPrice - 30)
                    buy_order += single_volume
                    td.PrepareOrder(tick.InstrumentID, s, pj, single_volume, tick.LastPrice + 30)
