...
IFs = IF1104,IF1105,IF1901,IF1109
CFs = CF109,CF107
...
class MdSpiDelegate(MdApi):
    ...
    def checkErrorRspInfo(self, info):
        if info.ErrorID != 0:
            logger.error("MD:ErrorID:%s,ErrorMsg:%s" % (info.ErrorID, info.ErrorMsg))
    ...
    def OnRspUserLogin(self, userlogin, info, rid, is_last):
        self.logger.info('MD:user login,info:%s,rid:%s,is_last:%s' % (info, rid, is_last))
        scur_day = int(time.strftime('%Y%m%d'))
        if scur_day > self.agent.scur_day:  # 换日,重新设置volume
            self.logger.info('MD:换日, %s-->%s' % (self.agent.scur_day, scur_day))
    ...
    def OnRtnDepthMarketData(self, depth_market_data):
        ...
        try:
            ...
            if depth_market_data.LastPrice > 999999 or depth_market_data.LastPrice < 10:
                self.logger.warning(
                    'MD:收到的行情数据有误:%s,LastPrice=:%s' % (depth_market_data.InstrumentID, depth_market_data.LastPrice))
            ...
            self.logger.debug('MD:收到行情，inst=%s,time=%s，volume=%s,last_volume=%s' % (
            dp.InstrumentID, dp.UpdateTime, dp.Volume, self.last_map[dp.InstrumentID]))
            if dp.Volume <= self.last_map[dp.InstrumentID]:
                self.logger.debug('MD:行情无变化，inst=%s,time=%s，volume=%s,last_volume=%s' % (
                dp.InstrumentID, dp.UpdateTime, dp.Volume, self.last_map[dp.InstrumentID]))
                return  # 行情未变化
        ...
        if self.agent.save_flag == True:
            ...
            try:
                ff.write(
                    '%(instrument)s,%(date)s,%(min1)s,%(sec)s,%(msec)s,%(holding)s,%(dvolume)s,%(price)s,%(high)s,%(low)s,%(bid_price)s,%(bid_volume)s,%(ask_price)s,%(ask_volume)s\n' % ctick.__dict__)
    ...
    def market_data2tick(self, market_data):
        ...
        try:
            # rev的后四个字段在模拟行情中经常出错
            rev = BaseObject(instrument=market_data.InstrumentID, date=self.agent.scur_day, bid_price=0, bid_volume=0,
            ...
            self.logger.warning('MD:%s 行情数据转换错误:%s,updateTime="%s",msec="%s",tday="%s"' % (
            market_data.InstrumentID, str(inst), market_data.UpdateTime, market_data.UpdateMillisec,
...
class TraderSpiDelegate(TraderApi):
    ...
    def query_settlement_info(self):
        ...
        self.logger.info('TD:取上一日结算单信息并确认,BrokerID=%s,investorID=%s' % (self.broker_id, self.investor_id))
        req = ApiStruct.QrySettlementInfo(BrokerID=self.broker_id, InvestorID=self.investor_id, TradingDay='')
    ...
    def OnRspQrySettlementInfoConfirm(self, pSettlementInfoConfirm, pRspInfo, nRequestID, bIsLast):
        '''请求查询结算信息确认响应'''
        self.logger.debug("TD:结算单确认信息查询响应:rspInfo=%s,结算单确认=%s" % (pRspInfo, pSettlementInfoConfirm))
        # self.query_settlement_info()
        if (self.resp_common(pRspInfo, bIsLast, '结算单确认情况查询') > 0):
            if (pSettlementInfoConfirm == None or int(pSettlementInfoConfirm.ConfirmDate) < self.agent.scur_day):
                ...
                if (pSettlementInfoConfirm != None):
                    self.logger.info('TD:最新结算单未确认，需查询后再确认,最后确认时间=%s,scur_day:%s' % (
                    pSettlementInfoConfirm.ConfirmDate, self.agent.scur_day))
                else:
                    self.logger.info('TD:结算单确认结果为None')
                self.query_settlement_info()
            else:
                self.agent.isSettlementInfoConfirmed = True
                self.logger.info('TD:最新结算单已确认，不需再次确认,最后确认时间=%s,scur_day:%s' % (
                pSettlementInfoConfirm.ConfirmDate, self.agent.scur_day))
                self.agent.initialize()
    ...
    def OnRspSettlementInfoConfirm(self, pSettlementInfoConfirm, pRspInfo, nRequestID, bIsLast):
        ...
        if (self.resp_common(pRspInfo, bIsLast, '结算单确认') > 0):
            self.agent.isSettlementInfoConfirmed = True
            self.logger.info(
                'TD:结算单确认时间: %s-%s' % (pSettlementInfoConfirm.ConfirmDate, pSettlementInfoConfirm.ConfirmTime))
        self.agent.initialize()
    ...
    def OnRspQryOrder(self, pOrder, pRspInfo, nRequestID, bIsLast):
        ...
        if bIsLast and self.isRspSuccess(pRspInfo):
            ...
            self.logger.error('TD:requestID:%s,IsLast:%s,info:%s' % (nRequestID, bIsLast, str(pRspInfo)))
            pass
    ...
    def OnRspOrderInsert(self, pInputOrder, pRspInfo, nRequestID, bIsLast):
        ...
        self.logger.warning('TD:CTP报单录入错误回报, 正常后不应该出现,rspInfo=%s' % (str(pRspInfo),))
    ...
    def OnErrRtnOrderInsert(self, pInputOrder, pRspInfo):
        ...
        '''
        print('ERROR Order Insert')
        self.logger.warning('TD:交易所报单录入错误回报, 正常后不应该出现,rspInfo=%s' % (str(pRspInfo),))
        self.agent.err_order_insert(pInputOrder.OrderRef, pInputOrder.InstrumentID, pRspInfo.ErrorID, pRspInfo.ErrorMsg)
    ...
    def OnRspOrderAction(self, pInputOrderAction, pRspInfo, nRequestID, bIsLast):
        ...
        self.agent.err_order_action(pInputOrderAction.OrderRef, pInputOrderAction.InstrumentID, pRspInfo.ErrorID,
                                    pRspInfo.ErrorMsg)
    ...
    def OnErrRtnOrderAction(self, pOrderAction, pRspInfo):
        ...
        self.agent.err_order_action(pOrderAction.OrderRef, pOrderAction.InstrumentID, pRspInfo.ErrorID,
                                    pRspInfo.ErrorMsg)
...
class c_instrument(object):
    ...
    def create_instruments(names, strategy, t2order=t2order_if):
        ...
        for item in list(strategy.values()):
            if item.name not in objs:
                logging.warning('策略针对合约%s不在盯盘列表中' % (item.name,))
                continue
    ...
    def initialize_positions(self):  # 根据策略初始化头寸为0
        self.position_detail = dict([(ss.name, strategy.Position(self, ss)) for ss in list(self.strategy.values())])

    def calc_remained_volume(self):  # 计算剩余的可开仓量
        if self.cur_vtimes >= self.max_vtimes:  # 超过日开仓次数限制
            logging.info('超过日开仓手次限制:%s|%s' % (self.cur_vtimes, self.max_vtimes))
            return 0
        ...
        locked_volume = 0
        opened_volume = 0
        for position in list(self.position_detail.values()):
            plocked, popened = position.get_locked_volume()
            locked_volume += plocked
            opened_volume += popened
            logging.debug('计算策略锁定量: pos:%s,locked_volume=%s,opened_volume=%s' % (position, plocked, popened))
        remained_volume = self.max_volume - locked_volume if self.max_volume > locked_volume else 0
        logging.info('A_CRV2:%s合约总锁定数=%s,合约最大允许数=%s,剩余可开仓数=%s,已开仓数=%s' % (
        self.name, locked_volume, self.max_volume, remained_volume, opened_volume))
    ...
    def make_target_price(self, price, direction):
        ...
        '''
        return (
                   price + SLIPPAGE_BASE * self.tick_base if direction == LONG else price - SLIPPAGE_BASE * self.tick_base) / 10.0
    ...
    def day_switch(self):
        self.cur_vtimes = 0
        self.cur_profit = 0
        for ss in list(self.strategy.values()):  # 重新初始化opener
    ...
    def add_profit(self, profit):
        ...
        logging.info('当前利润:%s' % (self.cur_profit,))
...
class AbsAgent(object):
    ...
    def put_command(self, trigger_tick, command):  # 按顺序插入
        ...
        logging.debug(
            'AA_P:trigger_tick=%s,cticks=%s,len(command)=%s' % (trigger_tick, str(cticks), len(self.commands)))
    ...
    def check_commands(self):
        ...
        while (i < l and self.tick >= self.commands[i][0]):
            logging.info('AA_C:exec command,i=%s,tick=%s,command[i][0]=%s' % (i, self.tick, self.commands[i][0]))
            self.commands[i][1]()
            i += 1
        if i > 0:
...
class Agent(AbsAgent):
    ...
    def check_qry_commands(self):
        ...
        logging.debug('查询命令序列长度:%s' % (len(self.qry_commands),))
    ...
    def prepare_data_env(self):
        ...
        for hdata in list(hdatas.values()):
    ...
    def fetch_investor_position(self, instrument_id):
        ...
        req = ApiStruct.QryInvestorPosition(BrokerID=self.cuser.broker_id, InvestorID=self.cuser.investor_id,
    ...
    def fetch_investor_position_detail(self, instrument_id):
        ...
        '''
        logging.info('A:获取合约%s的当前持仓..' % (instrument_id,))
        req = ApiStruct.QryInvestorPositionDetail(BrokerID=self.cuser.broker_id, InvestorID=self.cuser.investor_id,
                                                  InstrumentID=instrument_id)
    ...
    def RtnTick(self, ctick):  # 行情处理主循环
        ...
        if self.trader != None and not self.trader.is_logged:
            logging.info('trader not logging,try login.......')
            self.trader.login()
    ...
    def prepare_tick(self, ctick):
        ...
        if inst not in self.instruments:
            logger.info('接收到未订阅的合约数据:%s' % (inst,))
            return False
        dinst = self.instruments[inst]  # .data
        ctick.iorder = dinst.get_order(ctick.min1)
        if (
                ctick.iorder < dinst.data.cur_min.viorder and ctick.date == dinst.data.cur_min.vdate) or ctick.date < dinst.data.cur_min.vdate:
            # print ctick.date,ctick.time,dinst.data.cur_min.vdate,dinst.data.cur_min.vtime
            logging.info('过滤:time=%s,ctick.iorder=%s,ctick.date=%s,ddc.viorder=%s,ddc.vdate=%s' % (
            ctick.time, ctick.iorder, ctick.date, dinst.data.cur_min.viorder, dinst.data.cur_min.vdate))
            return False
    ...
    def day_finalize(self, dinst, last_min, last_sec):
        ...
        with self.lock:
            ...
            try:
                ...
                last_current_time = 0
            logging.info('A_DF:cur_time=%s,last_min=%s' % (last_current_time, last_min))
    ...
    def prepare_base(self, dinst, ctick):
        ...
        if (ctick.iorder == ddata.cur_min.viorder + 1 and (
                ctick.sec > 0 or ctick.msec > 0)) or ctick.iorder > ddata.cur_min.viorder + 1 or ctick.date > ddata.cur_min.vdate:
            ...
            if (len(ddata.stime) > 0 and (ctick.date > ddata.sdate[-1] or ctick.min1 > ddata.stime[-1])) or len(
                    ddata.stime) == 0:  # 已有分钟与已保存的有差别
        ...
        elif ctick.iorder == ddata.cur_min.viorder or (
                ctick.iorder == ddata.cur_min.viorder + 1 and ctick.sec == 0 and ctick.msec == 0):  # 当分钟的处理. 在接续时，如果resume时间正好是当分钟，会发生当分钟重复计数
    ...
    def check_close_signal(self, ctick):
        ...
        if ctick.instrument not in self.instruments:
            logging.warning('需要监控的%s未记录行情数据')
            print('需要监控的%s未记录行情数据')
            return signals
        cur_inst = self.instruments[ctick.instrument]
        is_touched = False  # 止损位变化
        for position in list(cur_inst.position_detail.values()):
            for order in position.orders:
                if order.opened_volume > 0 and order.close_lock == False:
                    ...
                    if mysignal[0] != 0 and order.close_lock == False:  # 平仓
                        logging.info('平仓信号,time=%s,inst=%s,cur_price=%s,base_price=%s' % (
                        ctick.time, cur_inst.name, ctick.price, mysignal[1]))
    ...
    def check_open_signal(self, ctick):
        ...
        signals = []
        if ctick.instrument not in self.instruments:
            logging.warning('A_COS:需要监控的%s未记录行情数据')
            return signals
        cur_inst = self.instruments[ctick.instrument]
        for ss in list(cur_inst.strategy.values()):
    ...
    def lock_open_volume(self, instrument, order):
        ...
        '''
        logging.info('计算并虚拟锁定实际开仓数')
        want_volume = order.position.calc_open_volume()
        inst_remained = order.instrument.calc_remained_volume()
        logging.info('A_COV1:want_volume=%s,inst_remained=%s' % (want_volume, inst_remained))
        ...
        logging.info('A_COV2:理论开仓数:%s,单手保证金:%s' % (want_volume, margin_amount))

        if margin_amount <= 1:  # 不可能只有1块钱
            logging.error('合约%s保证金率未初始化' % (instrument.name,))
            return 0
        available_volume = int(self.available / margin_amount)
        logging.debug('A_COV3:可用保证金=%s,单手保证金=%s' % (int(self.available), margin_amount))
        if available_volume == 0:
            return 0
        logging.info('A_COV4:want_volume=%s,avaliable_volume=%s' % (want_volume, available_volume))
    ...
    def make_command(self, orders):
        ...
        for order in orders:
            ...
            if order.action_type == XOPEN:  ##开仓情况,X跳后不论是否成功均撤单
                ...
                if order.volume == 0:
                    logging.info('策略%s,可下单数=0,中止' % (order.position,))
                    continue
                ##有效Order
                self.actions.append(('open', order.mytime, order.direction, order.get_strategy_name(), order.base_price,
                                     order.volume, order.target_price))
                ...
                logging.info('A_MC:当前tick=%s,下单有效时长(超过该时长未成交则撤单)=%s' % (
                self.get_tick(), order.position.strategy.opener.valid_length,))
                self.put_command(self.get_tick() + order.position.strategy.opener.valid_length,
                                 fcustom(self.cancel_command, command=command))
                logging.info('A_MC_B:设置开仓撤单完成,cur_tick=%s,触发点=%s' % (
                self.get_tick(), self.get_tick() + order.position.strategy.opener.valid_length))
                self.open_position(command)
            else:  # 平仓, Y跳后不论是否成功均撤单, 撤单应该比开仓更快，避免追不上
                # print u'平仓'
                # self.ref2order[command.order_ref] = order.source_order
                logging.info('A_MC_C:平仓处理')
                self.actions.append(('close', order.mytime, order.direction, order.source_order.get_strategy_name(),
                                     order.base_price, order.volume, order.target_price))
                ...
                self.put_command(self.get_tick() + order.source_order.get_stop_valid_length(),
                                 fcustom(self.cancel_command, command=command))
                self.put_command(self.get_tick() + order.source_order.get_stop_valid_length() + 1,
                                 lambda: order.source_order.release_close_lock())
                logging.info('A_MC_D:设置平仓撤单完成,cur_tick=%s,触发点=%s' % (
                self.get_tick(), self.get_tick() + order.source_order.get_stop_valid_length()))
                # logging.info(u'发出平仓指令，cur_tick=%s,释放锁的时间是=%s' % (self.get_tick(),self.get_tick()+order.source_order.get_stop_valid_length()+1))
                logging.info('发出平仓指令，cur_tick=%s,price=%s,释放锁的时间是=%s' % (
                self.get_tick(), command.price, self.get_tick() + order.source_order.get_stop_valid_length() + 1))
    ...
    def open_position(self, order):
        ...
        logging.info('下单: instrument=%s,方向=%s,数量=%s,价格=%s' % (
        order.instrument, '多' if order.direction == ApiStruct.D_Buy else '空', order.volume, order.price))
    ...
    def close_position(self, order, CombOffsetFlag=ApiStruct.OF_CloseToday):
        ...
        logging.info('平仓: sday=%s,cday=%s' % (sday, cday))
    ...
    def save_state(self):
        ...
        for inst in list(self.instruments.values()):
            for position in list(inst.position_detail.values()):
        ...
        for inst, orders in list(cur_orders.items()):
            cin = BaseObject(instrument=inst, opened_volume=sum([order.opened_volume for order in orders]),
                             orders=orders)
    ...
    def resume(self):
        ...
        cposs = set([])
        for chd in list(state.holdings.values()):
            ...
            for order in chd.orders:
                ...
                xorder = strategy.Order(cur_position, order.base_price, order.target_price, order.mytime,
                                        order.action_type)
        ...
        ##resume opener
        for cur_inst in list(self.instruments.values()):
            for ss in list(cur_inst.strategy.values()):
    ...
    def rtn_trade(self, strade):
        ...
        '''
        logging.info('A_RT1:成交回报,%s:orderref=%s,orders=%s,price=%s' % (
        strade.InstrumentID, strade.OrderRef, self.ref2order, strade.Price))
        if int(strade.OrderRef) not in self.ref2order or strade.InstrumentID not in self.instruments:
            logging.warning('A_RT2:收到非本程序发出的成交回报:%s-%s' % (strade.InstrumentID, strade.OrderRef))
        ...
        if myorder.action_type == XOPEN:  # 开仓, 也可用pTrade.OffsetFlag判断
            is_completed = myorder.on_trade(price=int(strade.Price * 10 + 0.1), volume=strade.Volume,
                                            trade_time=strade.TradeTime)
            logging.info('A_RT31,开仓回报');
            cur_inst.add_vtimes(strade.Volume)
        else:
            myorder.source_order.on_close(price=int(strade.Price * 10 + 0.1), volume=strade.Volume,
                                          trade_time=strade.TradeTime)
            logging.info('A_RT32,平仓回报,price=%s,time=%s' % (strade.Price, strade.TradeTime));
    ...
    def err_order_action(self, order_ref, instrument_id, error_id, error_msg):
        ...
        '''
        logging.info('撤单时已成交，error_id=%s,error_msg=%s' % (error_id, error_msg))
...
class SaveAgent(Agent):
    ...
    def RtnTick(self, ctick):  # 行情处理主循环
        inst = ctick.instrument
        if inst not in self.instruments:
            logging.info('接收到未订阅的合约数据:%s' % (inst,))
            return False
        if ctick.min1 > LAST_TRADE_TIME:
            print(('接收到不在时间范围的合约数据:%s:%s' % (inst, ctick.min1)))
            logging.info('接收到不在时间范围的合约数据:%s:%s' % (inst, ctick.min1))
            # return False
        dinst = self.instruments[inst]  # .data
        print('mytest:', inst, 'ctime.min1:', ctick.min1)
        ctick.iorder = dinst.get_order(ctick.min1)
        if (
                ctick.iorder < dinst.data.cur_min.viorder and ctick.date == dinst.data.cur_min.vdate) or ctick.date < dinst.data.cur_min.vdate:
            # print ctick.date,ctick.time,dinst.data.cur_min.vdate,dinst.data.cur_min.vtime
...
def save_raw(base_name='base.ini', strategy_name='strategy.ini', base='Base', strategy='strategy'):
    ...
    '''
    logging.basicConfig(filename="ctp_user_agent.log", level=logging.INFO,
...
def save(base_name='base.ini', strategy_name='strategy.ini', base='Base', strategy='strategy'):
    '''
        ...
        用到了User和Trader
    '''
    logging.basicConfig(filename="ctp_user_agent.log", level=logging.DEBUG,
                        format='%(name)s:%(funcName)s:%(lineno)d:%(asctime)s %(levelname)s %(message)s')

    cfg = config.parse_base(base_name, base)
    strategy_cfg = config.parse_strategy(strategy_name, strategy)

    ctrader = list(cfg.traders.values())[0]
...
def create_trader(name='base.ini', base='Base', sname='strategy.ini', t2order=t2order_if):
    logging.basicConfig(filename="ctp_trade.log", level=logging.DEBUG,
                        format='%(name)s:%(funcName)s:%(lineno)d:%(asctime)s %(levelname)s %(message)s')
    ...
    # 模拟trader
    cuser = list(cfg.traders.values())[0]
    logging.info('broker_id=%s,investor_id=%s,passwd=%s' % (cuser.broker_id, cuser.investor_id, cuser.passwd))
