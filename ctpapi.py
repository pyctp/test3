...
class MyMdApi(MdApi):
    ...
    def OnFrontConnected(self):
        print("connecting to the marketdata server ...")
    ...
    def OnRspUserLogin(self, user_login, info, rid, is_last):
        ...
        if is_last and not self.is_error_rsp_info(info):
            print(datetime.datetime.now(), '行情服务器登录成功。订阅合约', self.instruments)
...
class MyTradeApi(TraderDelegate):
    ...
    def OnRspUserLogin(self, user_login, info, rid, is_last):
        print(datetime.datetime.now(), "登录交易服务器...", is_last, info.ErrorMsg.decode('gbk'))
        print(datetime.datetime.now(),'user_login:', user_login)
        print(datetime.datetime.now(),info)
        print(datetime.datetime.now(),rid)

        q_server_info.put(copy.deepcopy(info))

        if is_last and not self.is_error_rsp_info(info):
            print(datetime.datetime.now(), '交易服务器登录成功。', user_login.BrokerID, user_login.UserID)
    ...
    def OnFrontDisconnected(self, nReason):
        ...
        print(datetime.datetime.now(), 'Trading server connect interrupted...', hex(nReason))
