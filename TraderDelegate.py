...
class agent(Agent):
    ...
    def put_command(self,trigger_tick,command): #按顺序插入
        ...
        logging.debug('AA_P:trigger_tick=%s,cticks=%s,len(command)=%s' % (trigger_tick,str(cticks),len(self.commands)))
...
class TraderDelegate(TraderApi):
    ...
    def OnRtnTrade(self, pTrade):
        '''成交通知'''
        # print pTrade
        self.logger.info('TD:成交通知,BrokerID=%s,BrokerOrderSeq = %s,exchangeID=%s,OrderSysID=%s,TraderID=%s, OrderLocalID=%s' %(pTrade.BrokerID,pTrade.BrokerOrderSeq,pTrade.ExchangeID,pTrade.OrderSysID,pTrade.TraderID,pTrade.OrderLocalID))
