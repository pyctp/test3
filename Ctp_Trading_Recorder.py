...
def tradingRecorder():
    ...
    logfilename = BROKER_ID + '_' + INVESTOR_ID + '_交易日志' + str(tradingday) + '.log'

    # 初始化日志文件
    with open(logfilename, 'w') as f:  #对于夜盘品种如果启动程序时间不对可能有问题，是否用tradingday???
    ...
    while True:
        ...
        if not q_rtn_order.empty():
            ...
            print(rtn_order.InstrumentID, rtn_order.Direction, rtn_order.CombOffsetFlag, rtn_order.CombHedgeFlag,rtn_order.LimitPrice, rtn_order.VolumeTotalOriginal,rtn_order.OrderSubmitStatus, rtn_order.VolumeTraded,rtn_order.VolumeTotal, rtn_order.InsertTime, end=' ')
            # orderList.append(rtn_order), '\n'
            with open(logfilename, 'a') as f:
                f.writelines(str(rtn_order) + '\n')
            # if len(orderList) > 0:
            #     with open(logfilename, 'a') as f:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        today = datetime.now().strftime("%Y-%m-%d")
        # logfilename = BROKER_ID + '_' + INVESTOR_ID + u'_交易日志' + str(today) + '.log'
        currentTime = datetime.now().time()
